...
- **JPA OSIV**
  ...
  Spring JPA를 사용할 때 OSIV(Open Session In View)의 **기본값은 true**입니다.

  OSIV가 활성화되어 있으면 외부 API와 DB 트랜잭션을 분리하더라도 HTTP 응답이 완료될 때까지 DB 커넥션을 점유하기 때문에 분리 이점을 얻을 수 없습니다. 따라서 반드시 **OSIV를 false로 설정**해야 합니다.
```yml
spring:
    jpa:
      open-in-view: false
