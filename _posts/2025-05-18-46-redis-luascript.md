...
서비스의 대규모 트래픽 처리를 위해서는 여러 서버로 구성된 분산 시스템 환경을 구축하는 것이 일반적입니다. 하지만 분산환경에서는 동시성 제어가 어려워지는데  이 글에서는 선착순 쿠폰 발금 시스템을 예시로 분산 시스템 환경에서 Redis를 활용한 동시성 처리 방법에 대해 살펴보겠습니다.
...
데이터 정합성이 보장되지만 트랜잭션 처리 및 디스크 접근으로 인한 성능 저하와 데드락이 발생 할 수 있습니다.

1. **Redis를 활용한 분산 락**:
    - 장점: 높은 성능, 원자적 연산 지원, 분산 환경에서도 동시성 제어 가능
    - 단점: 추가 인프라 필요, 구현 복잡도 증가


## 동시성 제어에 Redis가 사용되는 이유
...
추가 인프라가 필요하고 시스템 복잡성이 증가하는 단점이 있을 수 있지만 위와 같은 이유로 분산 환경에서 동시성 제어에 Redis가 많이 활용되고 있습니다.
